...
import { Player, Team } from "../db"; // Adjust the path accordingly
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    if (!playerNames || !numberOfTeams) {
      return res
        .status(400)
        .json({ error: "Both playerNames and numberOfTeams are required." });
    }
    ...
    res.status(201).json({
      message: "Players and teams created successfully.",
      players,
      teams,
    });
  } catch (error) {
